...
class IndividualReview extends React.Component {
  ...
  formatDate(date) {
    let d = new Date(date)
    let formattedDate = d.toLocaleString("en", { month: "long"  }) + ' ' +
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <div id='reviewHeader'>
          <div id='profileInfo'>
            <img src={`${this.props.review.profilepicture}${Math.random()}`}></img>
        ...
        <div id='reviewAuthor'>
          By <a href=''>{this.props.review.username}</a> on {this.formatDate(this.props.review.date)}
          {verifiedDiv}
        </div>
          <br></br>
          <div id='reviewText'>
          ...
          <div id='helpfulCount'>
            {this.state.helpful ? this.state.helpful : this.props.review.helpful} people found this helpful
          </div>
          <div id='reviewFooter'>
          <Button className='helpfulButton' bsStyle='default' disabled={this.state.disabled} onClick={(e) => this.handleButton(e)}>
            Helpful
          </Button>
          <i className='verticalSeperator'></i>
